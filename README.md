...
<img src="https://capsule-render.vercel.app/api?type=rect&color=0:6b46c1,100:2b6cb0&height=120&section=header&text=BYTEMATCH&fontSize=48&fontColor=ffffff&fontAlignY=58" width="100%" alt="BYTEMATCH"/>

# BYTEMATCH

### Verifies that deployed on-chain bytecode matches a given source/Foundry build, detecting unverified or tampered proxies and implementations.

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=18&duration=3500&pause=1000&color=6B46C1&center=true&vCenter=true&width=720&lines=Verifies+that+deployed+onchain+bytecode+matches+a+given+sour;Self-hostable+%C2%B7+MCP-native+%C2%B7+CI-ready+%C2%B7+polyglot" width="720"/>

[![PyPI](https://img.shields.io/pypi/v/cognis-bytematch.svg?color=6b46c1)](https://pypi.org/project/cognis-bytematch/) [![CI](https://github.com/cognis-digital/bytematch/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/bytematch/actions) [![License: COCL 1.0](https://img.shields.io/badge/License-COCL%201.0-2b6cb0.svg)](LICENSE) [![Suite](https://img.shields.io/badge/Cognis-Neural%20Suite-6b46c1.svg)](https://github.com/cognis-digital)
...
Decentralized 'is this contract actually what its repo claims' check that runs in CI/CD — supply-chain integrity for on-chain code.

`bytematch` is single-purpose, scriptable, and self-hostable: point it at a target, get prioritized results in the format your workflow already speaks (table · JSON · SARIF), gate CI on it, and let agents drive it over MCP.
...
- ✅ Extract Metadata
- ✅ Verify
- ✅ Load Artifact Runtime Bytecode
- ✅ Verify Artifact
...
bytematch scan . --fail-on high        # CI gate (non-zero exit)
```
...
```text
$ bytematch scan .
  [HIGH    ] BYT-001  example finding             (./src/app.py)
  [MEDIUM  ] BYT-002  another signal              (./config.yaml)
...
`bytematch` is interoperable with every popular way of using AI:

- **MCP server** — `bytematch mcp` (Claude Desktop, Cursor, Cognis.Studio, [uncensored-fleet](https://github.com/cognis-digital/uncensored-fleet))
- **OpenAI-compatible / JSON** — pipe `bytematch scan . --format json` into any agent or LLM
...
*Built in the spirit of **Sourcify**, re-framed the Cognis way. Missing a credit? Open a PR.*
...
Pipes into your stack: **SARIF** for code-scanning, **JSON** for anything, an **MCP server** (`bytematch mcp`) for AI agents, and a webhook forwarder for SIEM/Slack/Jira. See [`docs/INTEGRATIONS.md`](docs/INTEGRATIONS.md).
...
|---|---|---|---|---|
| `scripts/setup-linux.sh` | `scripts/setup-macos.sh` | `scripts/setup-windows.ps1` | `docker run ghcr.io/cognis-digital/bytematch` | [DEPLOY.md](docs/DEPLOY.md) (AWS/Azure/GCP/k8s) |
...
> ### ⭐ If `bytematch` saved you time, **star it** — it genuinely helps others find it.
