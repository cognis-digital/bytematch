...
How **bytematch** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `bytematch` lives in the **Malware / binary** cluster.
...
| from | relation | to |
|---|---|---|
| `bytematch` | JSON findings compose with | [`yaragen`](https://github.com/cognis-digital/yaragen), [`binhunt`](https://github.com/cognis-digital/binhunt), [`entropyscan`](https://github.com/cognis-digital/entropyscan) |
| `bytematch` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `bytematch` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `bytematch` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the malware / binary cluster.** `bytematch` output feeds the next tool:
```bash
bytematch ... --format json > out.json          # this tool's findings (see `bytematch --help`)
yaragen ... < out.json                      # the cluster sibling consumes them
```
...
bytematch ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
bytematch ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
bytematch ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `bytematch`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `bytematch` slots
