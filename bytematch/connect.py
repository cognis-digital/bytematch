"""Native cognis-connect emit for bytematch — forward findings to any platform.

Maps bytematch's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    bytematch ... --format json | bytematch-emit --to stix
    bytematch-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('summary', '')
        out['severity'] = 'info'
        out['type'] = 'bytecode_mismatch'
